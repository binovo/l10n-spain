...
from .ticketbai_response import TicketBaiResponseState
from .ticketbai_invoice import TicketBaiInvoiceState
...
class AccountInvoice(models.Model):
    ...
    @api.model
    def tbai_get_next_pending_invoice(self):
        """
        Get the first created tbai.invoice in pending state between the following:
          - Customer invoice
          - Customer invoice cancellation
        :return: tbai.invoice
        """
        customer_invoice = \
            self.env['tbai.invoice.customer.invoice'].get_next_pending_invoice()
        customer_cancellation = \
            self.env['tbai.invoice.customer.cancellation'].get_next_pending_invoice()
        sorted_invs = sorted(
            [x for x in [customer_invoice, customer_cancellation] if x],
            key=lambda x: x.create_date)
        return 0 < len(sorted_invs) and sorted_invs[0] or self.env['tbai.invoice']

    @api.model
    def _tbai_cancel_and_recreate_pending_invoices(self, company_id=False):
        self.env['tbai.invoice.customer.invoice'].cancel_and_recreate_pending_invoices(
            company_id=company_id
        )

    @api.model
    def _tbai_cancel_and_recreate_cancellation(self, company_id=False):
        cancellation_model = 'tbai.invoice.customer.cancellation'
        self.env[cancellation_model].cancel_and_recreate_pending_invoices(
            company_id=company_id
        )

    @api.model
    def tbai_send_pending_invoices(self):
        next_pending_invoice = self.tbai_get_next_pending_invoice()
        retry_later = False
        rejected_retries = 0
        while next_pending_invoice and not retry_later and \
                rejected_retries < TBAI_REJECTED_MAX_RETRIES:
            company = next_pending_invoice.company_id
            tbai_response = next_pending_invoice.send()
            if TicketBaiResponseState.RECEIVED.value == tbai_response.state:
                next_pending_invoice.state = TicketBaiInvoiceState.sent.value
                ...
                # successfully sent.
                # Cancel and recreate all pending invoices.
                if 'tbai.invoice.customer.invoice' == next_pending_invoice._name:
                    company.tbai_last_invoice_id = \
                        company.tbai_last_invoice_id.tbai_previous_invoice_id
                    self._tbai_cancel_and_recreate_pending_invoices(
                        company_id=company.id)
                elif 'tbai.invoice.customer.cancellation' == next_pending_invoice._name:
                    self._tbai_cancel_and_recreate_cancellation(company_id=company.id)
                rejected_retries += 1
    ...
    @api.multi
    def tbai_rebuild_customer_invoice(self):
        self._tbai_customer_invoice()
    ...
    def _tbai_invoice_cancel(self):
        for record in self:
            tbai_invoice = record.env['tbai.invoice.customer.cancellation'].create({
                'invoice_id': record.id,
                'name': "%s - %s" % (_("Cancellation"), record.number),
                'company_id': record.company_id.id
            })
            tbai_invoice.build_tbai_invoice()
    ...
    @api.multi
    def tbai_rebuild_customer_cancellation(self):
        self._tbai_invoice_cancel()
...
class AccountInvoiceLine(models.Model):
    ...
    def tbai_get_value_cantidad(self):
        ...
        <element  name="Cantidad" type="T:ImporteSgn12.2Type"/>
            <pattern value="(\\+|-)?\\d{1,12}(\\.\\d{0,2})?"/>
        :return: Invoice Line Quantity (signed)
        """
    ...
    def tbai_get_value_descuento(self):
        ...
        <element  name="Descuento" type="T:ImporteSgn12.2Type" minOccurs="0"/>
            <pattern value="(\\+|-)?\\d{1,12}(\\.\\d{0,2})?"/>
        :return: Invoice Line Discount Amount
        """
        if self.discount:
            res = "%.2f" % (self.price_unit * self.discount / 100.0)
        else:
            res = '0.00'
        return res
    ...
    def tbai_get_value_importe_total(self):
        ...
        <element  name="ImporteTotal" type="T:ImporteSgn12.2Type"/>
            <pattern value="(\\+|-)?\\d{1,12}(\\.\\d{0,2})?"/>
        :return: Invoice Line Amount Total with taxes (signed)
        """
        if RefundTypeEnum.differences.value == self.invoice_id.tbai_refund_type:
            sign = -1
        else:
