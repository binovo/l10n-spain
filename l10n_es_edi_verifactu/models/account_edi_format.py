...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_es_verifactu_get_xml(self, invoice, cancel=False):
        ...
        errors = self._ensure_software_settings(invoice) + self._ensure_invoice_data(
            invoice
        )
        if errors:
            raise UserError(
    ...
    @staticmethod
    def _get_invoice_id(invoice, cancel=False):
    ...
    @staticmethod
    def _get_recipient(partner_id, partner_info):
        recipient = {
    ...
    @staticmethod
    def _get_es_vat_lines(vat_breakdown):
        subject_lines = []
        no_subject_lines = []
    ...
    @staticmethod
    def _get_eu_extra_com_vat_lines(vat_breakdown):
        raise NotImplementedError()

    @api.model
    def _get_vat_lines(self, invoice, vat_breakdown):
        if invoice.commercial_partner_id.country_id.code == "ES":
            return self._get_es_vat_lines(vat_breakdown)
        else:
            return self._get_eu_extra_com_vat_lines(vat_breakdown)

    @staticmethod
    def _get_previous_id(previous_invoice):
    ...
    def verifactu_xmlgen_prepare_json(self, invoice, cancel=False, attach=False):
        if invoice and not cancel:
            json_input = {
                "invoice": {
                    "issuer": self._get_issuer(invoice),
                    ...
                    "id": self._get_invoice_id(invoice, cancel),
                    "description": self._get_description(invoice),
                    "type": self._get_invoice_type(invoice),
                }
            }
            ...
            if invoice_info_list:
                ...
                tax_amount, vat_lines = self._get_vat_lines(
            ...
            if previous_invoice:
                json_input["previousId"] = self._get_previous_id(previous_invoice)
            if attach:
    ...
    @staticmethod
    def _ensure_software_settings(invoice):
        errors = []
    ...
    @staticmethod
    def _ensure_invoice_data(invoice):
        errors = []
    ...
    @staticmethod
    def _ensure_supported_invoice(invoice):
    ...
    def _check_move_configuration(self, invoice):
        ...
        if self.code != "es_verifactu" or invoice.country_code != "ES":
            return errors
        errors += self._ensure_software_settings(invoice)
        errors += self._ensure_invoice_data(invoice)
        if not errors and self.env.context.get("l10n_es_verifactu_get_xml", False):
            self._ensure_supported_invoice(invoice)
